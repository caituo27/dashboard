...
const sharedMock = vi.hoisted(() => ({ readConsumerMyTasks: vi.fn() }));
vi.mock("./consumerMock", async (importOriginal) => ({
  ...await importOriginal<typeof import("./consumerMock")>(),
  readConsumerMyTasks: sharedMock.readConsumerMyTasks
}));
...
describe("readAgentSnapshot", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    sharedMock.readConsumerMyTasks.mockResolvedValue([]);
  ...
  it("adds shared execution records without replacing real tasks", async () => {
    apiMocks.task.market.mockResolvedValue([]);
    apiMocks.task.recommendations.mockResolvedValue([]);
    apiMocks.agent.list1.mockResolvedValue([]);
    apiMocks.agent.current.mockResolvedValue(undefined);
    apiMocks.myTask.list.mockResolvedValue([{ id: "real-execution", taskId: "real-task", status: "RUNNING" }]);
    const generated = {id: "demo:execution:9000000001", taskId: "demo:task:1", status: "执行中"};
    sharedMock.readConsumerMyTasks.mockResolvedValue([generated]);
    const snapshot = await readAgentSnapshot();
    expect(snapshot.myTasks?.map(row => row.id)).toEqual(["real-execution", generated.id]);
    expect(sharedMock.readConsumerMyTasks).toHaveBeenCalledWith("13812345678");
  });
