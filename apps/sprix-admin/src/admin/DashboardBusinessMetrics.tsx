...
function WeeklyHeterogeneityChart({weeks}:{weeks:BusinessWeek[]}){
 ...
 return <div className="executive-chart"><div className="executive-chart-legend"><span><i className="is-mint is-line"/>周度异构性 Agent 占比</span></div><svg viewBox={`0 0 ${chart.width} ${chart.height}`} role="img" aria-label="周度异构性 Agent 占比"><Grid leftLabel="异构性占比"/><AxisTicks leftMaximum={maximum} leftFormat={value=>`${Math.round(value)}%`}/><polyline className="executive-chart-line is-mint" points={points}/>{weeks.map((week,index)=><circle key={week.label} className="executive-chart-dot is-mint" cx={xAt(index,weeks.length)} cy={y(week.heterogeneousRate*100,maximum)} r="4"/>)}<ChartNodeHits tooltips={weeks.map(week=>[`统计周：${week.label}`,`异构性 Agent 占比：${percent(week.heterogeneousRate)}`].join('\n'))}/>{weeks.map((week,index)=><text className="executive-axis-label" key={week.label} x={xAt(index,weeks.length)} y={chart.height-14} textAnchor="middle">{week.label}</text>)}</svg></div>;
}
