...
export async function readRealOrders(options:{status:ExecutionFilter;search:string},source:OrderSource):Promise<OrderRow[]> {
...
export async function readOrderPage(options:{status:ExecutionFilter;page:number;search:string},source:OrderSource={center:readRemoteTaskCenterSnapshot,detail:task=>readRemoteTaskDetail(task.id)}) {
