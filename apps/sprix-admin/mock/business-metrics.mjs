...
for(const [weekIndex,raw] of rawWeeks.entries()){
 ...
 for(const [typeIndex,type] of TASK_TYPE_NAMES.entries()){
  ...
  const modeCents=[...allocate(autoCents,Array.from({length:autoCount},(_,index)=>1+(hash(index+typeIndex*17,76)%11))),...allocate(totalCents-autoCents,Array.from({length:executionTotal-autoCount},(_,index)=>1+(hash(index+weekIndex*19,77)%11)))];
  let localExecution=0;
  for(const taskIndex of taskIndexes){const task=taskBlueprints[taskIndex];for(let slot=0;slot<task.executionTotal;slot++){const index=executionBlueprints.length,day=daySequence[localExecution];executionBlueprints.push({index,taskIndex,weekIndex,typeIndex,type,day,dayPosition:dailySeen[day]++,deliveryMode:localExecution<autoCount?'AGENT':'USER_MANUAL',acceptedCents:modeCents[localExecution]});task.executionIndexes.push(index);localExecution++;}taskExecutionIndexes.set(taskIndex,task.executionIndexes);}
 }
}
...
export function businessTaskRecord(index,patch={}){
...
export function businessTaskDetail(index,patch={}){
 const blueprint=taskBlueprints[index],task=businessTaskRecord(index,patch);if(!blueprint||!task)return null;const reviewing=(taskExecutionIndexes.get(index)??[]).map(executionIndex=>{const row=businessAcceptanceRecord(executionIndex);return {executionId:row.executionId,executionNo:row.executionNo,executionIndex,userName:row.userName,phone:row.phone,agentName:row.agentName,submittedAt:row.submittedAt,acceptanceStatus:row.acceptanceStatus,acceptanceScore:row.acceptanceScore,acceptanceSummary:row.acceptanceSummary,acceptanceIssues:row.acceptanceIssues,score:row.agentScore,currentNode:row.currentNode,progress:row.progress,appealStatus:'无申诉',settlementStatus:'待结算',completedAt:row.submittedAt};});const files=task.category==='数据标注'?dataAnnotationTaskAttachmentFiles(task.id,blueprint.typeSequence,task.reward,task.totalSlots,task.publishedAt):otherTaskReferenceAttachmentFiles(task.id,task.category,blueprint.typeSequence,task.reward,task.totalSlots,task.publishedAt),attachments=files.map(file=>Object.fromEntries(Object.entries(file).filter(([key])=>key!=='bytes'&&key!=='content')));return {task,attachments,operationLogs:[],records:{running:[],reviewing,terminated:[],completed:[]}};
}
