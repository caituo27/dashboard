...
生产环境的业务数据通过各 app 的 `src/services/sprixApi.ts` service adapter 接入真实后端，并通过 TanStack Query 刷新远端快照；前端不再维护 `src/data/mock.ts` 作为生产业务数据源，也不在生产环境通过本地状态模拟写操作成功。本地开发提供独立的 `/mock-api` 演示层，用于联调尚未具备完整后端数据的管理页面和 C 端流程。C 端生产构建关闭共用 Mock；管理端如需演示数据，必须显式部署 Mock 服务及同源代理。

当前 C 端登录弹窗已接支付宝扫码、微信扫码和手机号验证码登录；支付宝收款账户绑定走后端授权会话。管理后台待打款页已接单笔支付宝打款和打款结果查询，真实出款仍以后端支付宝配置和 `payout-enabled` 开关为准。

后端尚未提供的生产能力，前端保持空状态、禁用动作或“待接口接入”提示，不以本地 Mock 冒充线上结果。接单资格页已接入支付宝人脸核验初始化、实人核验完成确认和自由职业者协议签署接口；正式协议全文仍待后续替换当前占位正文。

- 智能接单状态机、阈值、命中任务和自动接单结果。
- 推荐/匹配能力和预计 Token 字段。
- C 端/后台 execution 完整详情、日志、交付文件、验收报告和结算明细。
- 后台结算详情、打款清单导出、资金批量操作和 C 端历史提现查询。
- Swagger 生成客户端尚未覆盖部分手写接口，包括后台账号密码登录和若干管理详情接口。
